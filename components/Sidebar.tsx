  ...
  return (
    <aside className={`hidden md:flex flex-col h-full glass-panel border-r-0 border-white/40 m-4 rounded-3xl shadow-glass z-20 transition-all duration-300 relative ${isCollapsed ? 'w-24' : 'w-72'}`}>
      ...
      <button 
        ...
        className="absolute -right-3 top-9 bg-white border border-gray-200 rounded-full p-1 shadow-sm text-gray-500 hover:text-blue-600 z-50"
      >
      ...
      <div className={`p-8 border-b border-white/20 flex ${isCollapsed ? 'justify-center px-2' : 'items-center gap-3'}`}>
        ...
        {!isCollapsed && <span className="text-xl font-extrabold tracking-tight text-slate-800 animate-in fade-in duration-200">ServiceCRM</span>}
      </div>
      ...
      <div className={`p-6 border-t border-white/20 text-xs text-slate-500 text-center font-medium ${isCollapsed ? 'hidden' : 'block'}`}>
        <div className="bg-white/30 rounded-xl py-2 backdrop-blur-sm">
          Liquid Glass v2.1
        </div>
      </div>
    </aside>
